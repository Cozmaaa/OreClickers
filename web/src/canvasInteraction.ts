import { Game, GameState } from "./app.js";
...
export class CanvasHandler {
    ...
    handleUserState(gameState: GameState, event: MouseEvent | KeyboardEvent) {
        switch (gameState) {
    ...
    handleMainMenuClicks(event: MouseEvent) {
        ...
        if (
            event.clientX >= createLobbyButton.x &&
        ...
        if (
            event.clientX >= joinLobbyButton.x &&
    ...
    handleInsertUsernameKeyboardEvents(event: KeyboardEvent) {
    ...
    handleInsertUsernameMouseEvents(event: MouseEvent) {
        const buttonWidth = 300;
    ...
    private checkIfButtonClicked(event: MouseEvent): void {
